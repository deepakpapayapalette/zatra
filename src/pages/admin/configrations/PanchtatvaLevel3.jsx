...
const PanchtatvaLevel3 = () => {
        ...
        const handleSubmit = async (e) => {
            ...
            if (!formData?.subCategoryName?.trim() && formData?.subCategoryName?.length === 0) {
                toast.error("Please enter a valid Panchtatva Level 3");
                return;
            }
            setIsLoading(true);
            try {
                const payload = {
                    ...
                    "lookup_type": "panchtatva_sub_sub_category",
                    "parent_lookup_type": "panchtatva_sub_category",
                    "parent_lookup_id": formData?.categoryId || null,
                }
                const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
                if (res.response && res.response.response_code === "200") {
                    toast.success(editId ? "Panchtatva Level 3 updated successfully" : "Panchtatva Level 3 added successfully");
                    setFormData({
                        subCategoryName: '',
                        categoryId: '',
                    });
                    setEditId(null);
                    setIsLoading(false);
                    fetchData(['panchtatva_sub_sub_category'], "subCategoryList");
                } else {
                    toast.error(res.response.response_message || "Failed to add Panchtatva Level 3");
                }
        ...
        useEffect(() => {
            fetchData(['panchtatva_sub_category'], "categoryList");
            fetchData(['panchtatva_sub_sub_category'], "subCategoryList");
        }, []);
            ...
            try {
                ...
                if (result.isConfirmed) {
                    ...
                    if (res?.response?.response_code === "200") {
                        toast.success("Panchtatva Level 3 deleted successfully");
                        fetchData(['panchtatva_sub_sub_category'], "subCategoryList");
                    }
                }
            } catch (error) {
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Panchtatva Level 3"
                description="Add or update the required details for the Panchtatva Level 3 to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-lg  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormInput
                        ...
                        name="categoryId"
                        placeholder="Select Panchtatva Level 2"
                        type="select"
                    ...
                    <FormInput
                        label="Panchtatva Level 3"
                        name="subCategoryName"
                        placeholder="Enter Panchtatva Level 3"
...
export default PanchtatvaLevel3