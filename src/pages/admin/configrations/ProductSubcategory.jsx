...
const ProductSubcategory = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData?.subCategoryName?.trim() && formData?.subCategoryName?.length === 0) {
            toast.error("Please enter a valid Product sub category");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_type": "product_sub_category",
                "parent_lookup_type": "product_category",
                "parent_lookup_id": formData?.categoryId || null,
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Product sub category updated successfully" : "Product sub category added successfully");
                setFormData({
                    subCategoryName: '',
                    categoryId: '',
                });
                ...
                fetchData(['product_sub_category'], "subCategoryList");
            } else {
                toast.error(res.response.response_message || "Failed to add Product sub category");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    useEffect(() => {
        fetchData(['product_category'], "categoryList");
        fetchData(['product_sub_category'], "subCategoryList");
    }, []);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Product sub category"
                description="Add or update the required details for the Product sub category to keep records accurate and complete."
            />
